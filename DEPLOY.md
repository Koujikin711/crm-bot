...
- **Веб-интерфейс:** приложение на Amvera запускается как **run_all.py** — в одном процессе работают и бот, и веб-дашборд. Открывай в браузере URL своего приложения Amvera (например `https://твой-проект.amvera.ru`) — там будет страница входа через Telegram. Для сессий задай в Amvera переменную `WEB_SECRET_KEY` (произвольная строка).
